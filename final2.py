...
def process_frame(frame):
    global processed_frame_queue, gender_count, incident_count
...
if len(sys.argv) > 1 and sys.argv[1] == '--external-camera':
...
cv2.namedWindow("Real-time Feed", cv2.WINDOW_NORMAL)
cv2.namedWindow("Processed Feed", cv2.WINDOW_NORMAL)
cv2.resizeWindow("Real-time Feed", 800, 600)
cv2.resizeWindow("Processed Feed", 800, 600)
...
while webcam.isOpened():
    ...
    if not processed_frame_queue.empty():
        overlay_frame = processed_frame_queue.get()
